...
class RBM(object):
    ...
    def train(self, X):
        ...
        update_vw = _vw * cfg.dbn_momentum + cfg.dbn_learning_rate *\
            (positive_grad - negative_grad) / tf.to_float(tf.shape(v0)[0])

        update_vvb = _vvb * cfg.dbn_momentum + \
            cfg.dbn_learning_rate * tf.reduce_mean(v0 - v1, 0)

        update_vhb = _vhb * cfg.dbn_momentum + \
            cfg.dbn_learning_rate * tf.reduce_mean(h0 - h1, 0)
        ...
        with tf.Session() as sess:
            ...
            old_hb = self.init_hb
            old_vb = self.init_vb
            for i in range(cfg.dbn_epoches):
                for start, end in zip(range(0, len(X), cfg.batch_size),
                                      range(cfg.batch_size,
                                            len(X), cfg.batch_size)):
                    batch = X[start:end]
                ...
                # img_shape = (int(math.ceil(math.sqrt(self._input_size))),
                #              int(math.ceil(math.sqrt(self._input_size))))
                # tile_shape = (int(math.ceil(math.sqrt(self._output_size))),
                #               int(math.ceil(math.sqrt(self._output_size))))
                # data_show = old_w.T
                ...
                #         tile_spacing=(1, 1),
                #         pading_X=((0, tile_shape[0] * tile_shape[1] - data_show.shape[0]),
                #                   (0, img_shape[0] * img_shape[1] - data_show.shape[1]))
                #     )
                # )
