    ...
    def __call__(self, input, kernel_size=None, stride=None):
        ...
        if self.layer_type == 'NN':
            if not self.with_routing:
                ...
                capsules = tf.reshape(input, shape=(cfg.batch_size, -1, self.vec_len, 1))
            ...
            if self.with_routing:
                ...
                with tf.variable_scope('routing'):
                    ...
                    b_IJ = tf.constant(np.zeros(
                        [cfg.batch_size, input.shape[1].value, self.num_outputs, 1, 1], dtype=np.float32))
                    capsules = self.routing(b_IJ)
                    capsules = tf.squeeze(capsules, axis=1)
                return(capsules)


    def routing(self, b_IJ):
        ...
        Notes:
         ...
         '''
        cur_caps_num = self.input.shape[1].value
        cur_caps_vec = self.input.shape[-2].value
        # W: [num_caps_i, num_caps_j, len_u_i, len_v_j]
        W = tf.get_variable('Weight', shape=(1, cur_caps_num, self.num_outputs, cur_caps_vec, self.vec_len),
                            dtype=tf.float32,
        ...
        input = tf.tile(self.input, [1, 1, self.num_outputs, 1, 1])
        ...
        for r_iter in range(cfg.iter_routing):
            with tf.variable_scope('iter_' + str(r_iter)):
                ...
                if r_iter == cfg.iter_routing - 1:
                    ...
                    v_J_tiled = tf.tile(v_J, [1, cur_caps_num, 1, 1, 1])
