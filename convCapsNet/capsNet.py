...
class CapsNet(object):
    ...
    def build_arch(self):
        conv1 = tf.layers.conv1d(tf.reshape(self.X, [-1,179,1]),# 将原来任意行位置（-1）179列*1维（1个通道）的向量进行变形
                                 filters= 32,#变为32个通道
                                 kernel_size=3,#卷积核大小一般设置为3最稳定
                                 padding = 'same',#补0
                                 kernel_regularizer=tf.contrib.layers.l2_regularizer(0.001),#进行正则化，减低过拟合
                                 bias_regularizer=tf.contrib.layers.l2_regularizer(0.001),#进行正则化，减低过拟合
                                 activation=tf.nn.relu #激活函数
                                 )

        conv1_bn = tf.layers.batch_normalization(conv1)#批量标准化
        conv1_reshape = tf.reshape(conv1_bn, [-1, 32*179])# 卷积变形为任意一行都是，32*179列的向量

        fc1 = tf.layers.dense(inputs= conv1_reshape,#将卷积变形的向量作为全连接的输入
                                   units = 128,
                                    activation=tf.nn.relu)

        fc2 = tf.layers.dense(inputs= fc1,#将FC1变形的向量作为全连接的输入
                                 units=64,#全连接有64个神经元
                                 activation=tf.nn.relu)
